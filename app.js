(() => {
  ...
  // ===================== Provider selector (Bitget fix) =====================
  function getInjectedProvider() {
    const eth = window.ethereum;

    // Multiple injected providers (mobile browsers often)
    if (eth?.providers?.length) {
      const bitget = eth.providers.find(p => p.isBitKeep || p.isBitgetWallet);
    ...
    // Some Bitget builds expose bitkeep directly
  ...
  let usdt = null, core = null, staking = null, binary = null, vault5 = null;
  let USDT_DEC = 18;
  ...
  function startTick() {
    ...
    tickTimer = setInterval(() => {
      ...
      if (stakeEndTs > 0) {
        ...
        setText("stakeEnd", when);
        setText("stakeCdText", left > 0
          ? `Stake ends at: ${when}`
          : `Stake matured ✅ Claim is available (end: ${when})`
        );
      } else {
      ...
      if (vaultEndTs > 0) {
        ...
        setText("vExpires", when);
        setText("vaultCdText", left > 0
          ? `Vault expiry at: ${when}`
          : `Expired ❌ (expiry: ${when})`
        );
      } else {
  ...
  function parseReferralFromURL() {
    ...
    const side = (u.searchParams.get("side") || "").trim().toUpperCase();

    urlSponsor = isAddr(ref) ? ref : "";
    urlSide = (side === "L" || side === "R") ? side : "";
  }

  function getSponsorInput() {
    return ($("inpSponsor")?.value || "").trim();
  }

  function setSponsorInput(val, lock) {
    const inp = $("inpSponsor");
    if (!inp) return;
    inp.value = val || "";
    inp.readOnly = !!lock;
    sponsorLocked = !!lock;
    inp.style.opacity = lock ? "0.92" : "1";
  }

  function applyReferralAutofill() {
    if (urlSponsor) {
      setSponsorInput(urlSponsor, true);
      toast("Sponsor loaded from referral link.", "ok");
    } else {
      setSponsorInput(getSponsorInput(), false);
    }

    if (urlSide === "L") sideRight = false;
    if (urlSide === "R") sideRight = true;

    updateRefPreview();
  }

  function buildRefLink(side) {
    const sponsor = userAddr || getSponsorInput() || urlSponsor || CFG.DEFAULT_SPONSOR;
    const base = `${window.location.origin}${window.location.pathname}`;
    return `${base}?ref=${sponsor}&side=${side}`;
  }
  ...
  async function shareLink(text) {
    try {
      if (navigator.share) {
        await navigator.share({ title: "365DF Referral", text, url: text });
      } else {
        await copyText(text);
      }
      toast("Referral link ready.", "ok");
    } catch {
      await copyText(text);
    }
  }

  // Make preview readable on mobile (multi-line)
  function updateRefPreview() {
    const l = buildRefLink("L");
    const r = buildRefLink("R");
    setText("refPreview", `LEFT:\n${l}\n\nRIGHT:\n${r}`);
  }
  ...
  // ===================== Connect (Bitget-stable) =====================
  async function connect() {
    const injected = getInjectedProvider();
    if (!injected) {
      toast("Wallet not found (Bitget / MetaMask).", "err");
      return;
    }
    ...
    try {
      ...
      // Bitget iOS is more stable if we request accounts first
      ...
      const wl = $("walletLink");
      if (wl) {
        wl.href = `${CFG.EXPLORER}/address/${userAddr}`;
        wl.textContent = shortAddr(userAddr);
      }

      // Ensure BSC after we have account
      ...
      setText("buyStatus", `Connect error: ${shortErr(e)}`);
  ...
  async function addTokens() {
    ...
    try {
      await window.ethereum.request({
        ...
        params: { type: "ERC20", options: { address: CFG.DF, symbol: "365DF", decimals: 18 } },
      });
      await window.ethereum.request({
        method: "wallet_watchAsset",
  ...
  async function buyOrUpgrade() {
    ...
    let sponsor = getSponsorInput();
    if (!sponsor) sponsor = CFG.DEFAULT_SPONSOR;

    if (!isAddr(sponsor)) {
      toast("Invalid sponsor address.", "err");
      setText("buyStatus", "Invalid sponsor address. Provide sponsor or use company sponsor.");
      return;
    }
    ...
    try {
      ...
      if (activeStake && selectedPkg <= currentPkg) {
        setText("buyStatus",
          "Blocked: ACTIVE_STAKE_EXISTS. Select a higher package to upgrade, or wait until maturity and claim."
        );
    ...
    try {
      ...
      await tx.wait();
      setText("buyStatus", "Success ✅ Refreshing...");
      ...
      if (String(msg).includes("ACTIVE_STAKE_EXISTS")) {
        setText("buyStatus",
          "Reverted: ACTIVE_STAKE_EXISTS. Upgrade with a higher package, or wait until stake maturity and claim."
        );
  ...
  async function refreshAll() {
    ...
    try {
      ...
      setText("pendingDF", fmtUnits(pend, 18, 4));

      const cU = await vault5.claimableUSDT(userAddr);
      const cD = await vault5.claimableDF(userAddr);
      setText("vClaimUSDT", fmtUnits(cU, USDT_DEC, 4));
      setText("vClaimDF", fmtUnits(cD, 18, 4));

      const e = await vault5.earns(userAddr);
      const lockUSDT = e.lockedUSDT;
      const lockDF = e.lockedDF;
      const endU = Number(e.lockEndUSDT);
      const endD = Number(e.lockEndDF);

      setText("vLocked", `${fmtUnits(lockUSDT, USDT_DEC, 4)} USDT / ${fmtUnits(lockDF, 18, 4)} DF`);
      vaultEndTs = Math.max(endU || 0, endD || 0);

      const vol = await binary.volumesOf(userAddr);
      setText("binLRP", `${vol.l} / ${vol.r} / ${vol.p}`);
  ...
  function bindPkgUI() {
    ...
    $("btnLeft")?.addEventListener("click", () => {
      sideRight = false;
      toast("Binary side: LEFT", "ok");
    });
    $("btnRight")?.addEventListener("click", () => {
      sideRight = true;
      toast("Binary side: RIGHT", "ok");
    });

    $("inpSponsor")?.addEventListener("input", () => {
      if (sponsorLocked) return;
      updateRefPreview();
    });

    $("btnCopyL")?.addEventListener("click", async () => copyText(buildRefLink("L")));
    $("btnShareL")?.addEventListener("click", async () => shareLink(buildRefLink("L")));
    $("btnCopyR")?.addEventListener("click", async () => copyText(buildRefLink("R")));
    $("btnShareR")?.addEventListener("click", async () => shareLink(buildRefLink("R")));
  }
  ...
  function bindUI() {
    ...
    // IMPORTANT: Do NOT hard reload on Bitget (causes "connect then disconnect")
    const injected = getInjectedProvider();
    if (injected?.on) {
  ...
  // ===================== Boot =====================
  // init countdown text safely
  setCountdown("stake", 0);
  setCountdown("vault", 0);
  setText("stakeCdText", "Connect wallet to load stake data.");
  setText("vaultCdText", "Connect wallet to load vault data.");

  // fill referral from URL
  parseReferralFromURL();
  applyReferralAutofill();
  updateRefPreview();
