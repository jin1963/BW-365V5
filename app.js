(() => {
  ...
  const CFG = {
    ...
    // Company/default sponsor (YOUR VALUE)
  ...
  let selectedPkg = 1;    // 1 small,2 med,3 large
  ...
  function buildRefLink(side) {
    // sponsor priority: connected wallet (share your link) -> input -> urlSponsor -> company
  ...
  function updateRefPreview() {
    ...
    setText("refPreview", `LEFT: ${l} | RIGHT: ${r}`);
  }
  ...
  async function buyOrUpgrade() {
    ...
    // Sponsor priority:
    // 1) if referral link ref exists -> input is locked already
    // 2) user typed input
    // 3) company sponsor fallback
    ...
    // Pre-check ACTIVE_STAKE_EXISTS behavior (avoid wasting tx)
    try {
      ...
      if (activeStake && selectedPkg <= currentPkg) {
        setText("buyStatus",
          "Blocked: ACTIVE_STAKE_EXISTS. You already have an active stake. Select a higher package to upgrade, or wait until maturity and claim."
        );
  ...
  function bindPkgUI() {
    ...
    $("inpSponsor")?.addEventListener("input", () => {
      // if locked, ignore edits
      if (sponsorLocked) return;
      updateRefPreview();
    });

    // referral buttons (if present)
  ...
  // Start with preview link even before connect
  updateRefPreview();
})();
